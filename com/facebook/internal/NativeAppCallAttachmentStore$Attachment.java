    ...
    if (paramUri != null)
    {
      ...
      if ("content".equalsIgnoreCase(paramBitmap))
      {
        ...
        if ((paramUri.getAuthority() != null) && (!paramUri.getAuthority().startsWith("media")))
        {
          ...
          label75: if (this.shouldCreateFile)
            break label201;
          paramBitmap = null;
          label84: this.attachmentName = paramBitmap;
          if (this.shouldCreateFile)
            break label211;
        }
      }
    }
    label201: label211: for (paramUUID = this.originalUri.toString(); ; paramUUID = FacebookContentProvider.getAttachmentUrl(FacebookSdk.getApplicationId(), paramUUID, this.attachmentName))
    {
