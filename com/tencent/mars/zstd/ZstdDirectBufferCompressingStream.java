      ...
      do
      {
        try
        {
          ...
          if (Zstd.isError(i))
          {
            ...
            localStringBuilder.<init>("Compression error: ");
            localIOException.<init>(Zstd.getErrorName(i));
        ...
        if (!this.target.isDirect())
        {
          ...
          ((IllegalArgumentException)localObject2).<init>("Target buffer should be a direct buffer");
          throw ((Throwable)localObject2);
        }
        if ((i > 0) && (!this.target.hasRemaining()))
        {
          localObject2 = new java/io/IOException;
          ((IOException)localObject2).<init>("The target buffer has no more space, even after flushing, and there are still bytes to compress");
          throw ((Throwable)localObject2);
        }
      }
      while (i > 0);
