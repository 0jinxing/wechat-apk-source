package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import e.a.a.b;
import java.util.LinkedList;

public class GetEmotionListRequest extends bsr
{
  public int Category;
  public SKBuiltinBuffer_t ReqBuf;
  public int ReqType;
  public int Scene;

  public final int op(int paramInt, Object[] paramArrayOfObject)
  {
    int i = 0;
    AppMethodBeat.i(62563);
    if (paramInt == 0)
    {
      paramArrayOfObject = (e.a.a.c.a)paramArrayOfObject[0];
      if (this.ReqBuf == null)
      {
        paramArrayOfObject = new b("Not all required fields were included: ReqBuf");
        AppMethodBeat.o(62563);
        throw paramArrayOfObject;
      }
      if (this.BaseRequest != null)
      {
        paramArrayOfObject.iy(1, this.BaseRequest.computeSize());
        this.BaseRequest.writeFields(paramArrayOfObject);
      }
      if (this.ReqBuf != null)
      {
        paramArrayOfObject.iy(2, this.ReqBuf.computeSize());
        this.ReqBuf.writeFields(paramArrayOfObject);
      }
      paramArrayOfObject.iz(3, this.ReqType);
      paramArrayOfObject.iz(4, this.Scene);
      paramArrayOfObject.iz(5, this.Category);
      AppMethodBeat.o(62563);
      paramInt = i;
      return paramInt;
    }
    if (paramInt == 1)
      if (this.BaseRequest == null)
        break label669;
    label669: for (paramInt = e.a.a.a.ix(1, this.BaseRequest.computeSize()) + 0; ; paramInt = 0)
    {
      i = paramInt;
      if (this.ReqBuf != null)
        i = paramInt + e.a.a.a.ix(2, this.ReqBuf.computeSize());
      paramInt = i + e.a.a.b.b.a.bs(3, this.ReqType) + e.a.a.b.b.a.bs(4, this.Scene) + e.a.a.b.b.a.bs(5, this.Category);
      AppMethodBeat.o(62563);
      break;
      if (paramInt == 2)
      {
        paramArrayOfObject = new e.a.a.a.a((byte[])paramArrayOfObject[0], unknownTagHandler);
        for (paramInt = bsr.getNextFieldNumber(paramArrayOfObject); paramInt > 0; paramInt = bsr.getNextFieldNumber(paramArrayOfObject))
          if (!super.populateBuilderWithField(paramArrayOfObject, this, paramInt))
            paramArrayOfObject.ems();
        if (this.ReqBuf == null)
        {
          paramArrayOfObject = new b("Not all required fields were included: ReqBuf");
          AppMethodBeat.o(62563);
          throw paramArrayOfObject;
        }
        AppMethodBeat.o(62563);
        paramInt = i;
        break;
      }
      if (paramInt == 3)
      {
        Object localObject1 = (e.a.a.a.a)paramArrayOfObject[0];
        GetEmotionListRequest localGetEmotionListRequest = (GetEmotionListRequest)paramArrayOfObject[1];
        paramInt = ((Integer)paramArrayOfObject[2]).intValue();
        int j;
        Object localObject2;
        boolean bool;
        switch (paramInt)
        {
        default:
          paramInt = -1;
          AppMethodBeat.o(62563);
          break;
        case 1:
          paramArrayOfObject = ((e.a.a.a.a)localObject1).Vh(paramInt);
          j = paramArrayOfObject.size();
          for (paramInt = 0; paramInt < j; paramInt++)
          {
            localObject2 = (byte[])paramArrayOfObject.get(paramInt);
            localObject1 = new hl();
            localObject2 = new e.a.a.a.a((byte[])localObject2, unknownTagHandler);
            for (bool = true; bool; bool = ((hl)localObject1).populateBuilderWithField((e.a.a.a.a)localObject2, (com.tencent.mm.bt.a)localObject1, bsr.getNextFieldNumber((e.a.a.a.a)localObject2)));
            localGetEmotionListRequest.BaseRequest = ((hl)localObject1);
          }
          AppMethodBeat.o(62563);
          paramInt = i;
          break;
        case 2:
          paramArrayOfObject = ((e.a.a.a.a)localObject1).Vh(paramInt);
          j = paramArrayOfObject.size();
          for (paramInt = 0; paramInt < j; paramInt++)
          {
            localObject2 = (byte[])paramArrayOfObject.get(paramInt);
            localObject1 = new SKBuiltinBuffer_t();
            localObject2 = new e.a.a.a.a((byte[])localObject2, unknownTagHandler);
            for (bool = true; bool; bool = ((SKBuiltinBuffer_t)localObject1).populateBuilderWithField((e.a.a.a.a)localObject2, (com.tencent.mm.bt.a)localObject1, bsr.getNextFieldNumber((e.a.a.a.a)localObject2)));
            localGetEmotionListRequest.ReqBuf = ((SKBuiltinBuffer_t)localObject1);
          }
          AppMethodBeat.o(62563);
          paramInt = i;
          break;
        case 3:
          localGetEmotionListRequest.ReqType = ((e.a.a.a.a)localObject1).BTU.vd();
          AppMethodBeat.o(62563);
          paramInt = i;
          break;
        case 4:
          localGetEmotionListRequest.Scene = ((e.a.a.a.a)localObject1).BTU.vd();
          AppMethodBeat.o(62563);
          paramInt = i;
          break;
        case 5:
          localGetEmotionListRequest.Category = ((e.a.a.a.a)localObject1).BTU.vd();
          AppMethodBeat.o(62563);
          paramInt = i;
          break;
        }
      }
      paramInt = -1;
      AppMethodBeat.o(62563);
      break;
    }
  }
}

/* Location:           C:\Users\Lin\Downloads\dex-tools-2.1-SNAPSHOT\dex-tools-2.1-SNAPSHOT\classes8-dex2jar.jar
 * Qualified Name:     com.tencent.mm.protocal.protobuf.GetEmotionListRequest
 * JD-Core Version:    0.6.2
 */