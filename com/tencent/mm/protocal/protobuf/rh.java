package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import e.a.a.b;
import java.util.LinkedList;

public final class rh extends bsr
{
  public String cvZ;
  public String fileid;
  public String vYy;
  public int vYz;

  public final int op(int paramInt, Object[] paramArrayOfObject)
  {
    int i = 0;
    AppMethodBeat.i(135754);
    if (paramInt == 0)
    {
      paramArrayOfObject = (e.a.a.c.a)paramArrayOfObject[0];
      if (this.fileid == null)
      {
        paramArrayOfObject = new b("Not all required fields were included: fileid");
        AppMethodBeat.o(135754);
        throw paramArrayOfObject;
      }
      if (this.cvZ == null)
      {
        paramArrayOfObject = new b("Not all required fields were included: md5");
        AppMethodBeat.o(135754);
        throw paramArrayOfObject;
      }
      if (this.BaseRequest != null)
      {
        paramArrayOfObject.iy(1, this.BaseRequest.computeSize());
        this.BaseRequest.writeFields(paramArrayOfObject);
      }
      if (this.fileid != null)
        paramArrayOfObject.e(2, this.fileid);
      if (this.cvZ != null)
        paramArrayOfObject.e(3, this.cvZ);
      if (this.vYy != null)
        paramArrayOfObject.e(4, this.vYy);
      paramArrayOfObject.iz(5, this.vYz);
      AppMethodBeat.o(135754);
      paramInt = i;
      return paramInt;
    }
    if (paramInt == 1)
      if (this.BaseRequest == null)
        break label656;
    label656: for (i = e.a.a.a.ix(1, this.BaseRequest.computeSize()) + 0; ; i = 0)
    {
      paramInt = i;
      if (this.fileid != null)
        paramInt = i + e.a.a.b.b.a.f(2, this.fileid);
      i = paramInt;
      if (this.cvZ != null)
        i = paramInt + e.a.a.b.b.a.f(3, this.cvZ);
      paramInt = i;
      if (this.vYy != null)
        paramInt = i + e.a.a.b.b.a.f(4, this.vYy);
      paramInt += e.a.a.b.b.a.bs(5, this.vYz);
      AppMethodBeat.o(135754);
      break;
      if (paramInt == 2)
      {
        paramArrayOfObject = new e.a.a.a.a((byte[])paramArrayOfObject[0], unknownTagHandler);
        for (paramInt = bsr.getNextFieldNumber(paramArrayOfObject); paramInt > 0; paramInt = bsr.getNextFieldNumber(paramArrayOfObject))
          if (!super.populateBuilderWithField(paramArrayOfObject, this, paramInt))
            paramArrayOfObject.ems();
        if (this.fileid == null)
        {
          paramArrayOfObject = new b("Not all required fields were included: fileid");
          AppMethodBeat.o(135754);
          throw paramArrayOfObject;
        }
        if (this.cvZ == null)
        {
          paramArrayOfObject = new b("Not all required fields were included: md5");
          AppMethodBeat.o(135754);
          throw paramArrayOfObject;
        }
        AppMethodBeat.o(135754);
        paramInt = i;
        break;
      }
      if (paramInt == 3)
      {
        Object localObject1 = (e.a.a.a.a)paramArrayOfObject[0];
        rh localrh = (rh)paramArrayOfObject[1];
        paramInt = ((Integer)paramArrayOfObject[2]).intValue();
        switch (paramInt)
        {
        default:
          paramInt = -1;
          AppMethodBeat.o(135754);
          break;
        case 1:
          paramArrayOfObject = ((e.a.a.a.a)localObject1).Vh(paramInt);
          int j = paramArrayOfObject.size();
          for (paramInt = 0; paramInt < j; paramInt++)
          {
            Object localObject2 = (byte[])paramArrayOfObject.get(paramInt);
            localObject1 = new hl();
            localObject2 = new e.a.a.a.a((byte[])localObject2, unknownTagHandler);
            for (boolean bool = true; bool; bool = ((hl)localObject1).populateBuilderWithField((e.a.a.a.a)localObject2, (com.tencent.mm.bt.a)localObject1, bsr.getNextFieldNumber((e.a.a.a.a)localObject2)));
            localrh.BaseRequest = ((hl)localObject1);
          }
          AppMethodBeat.o(135754);
          paramInt = i;
          break;
        case 2:
          localrh.fileid = ((e.a.a.a.a)localObject1).BTU.readString();
          AppMethodBeat.o(135754);
          paramInt = i;
          break;
        case 3:
          localrh.cvZ = ((e.a.a.a.a)localObject1).BTU.readString();
          AppMethodBeat.o(135754);
          paramInt = i;
          break;
        case 4:
          localrh.vYy = ((e.a.a.a.a)localObject1).BTU.readString();
          AppMethodBeat.o(135754);
          paramInt = i;
          break;
        case 5:
          localrh.vYz = ((e.a.a.a.a)localObject1).BTU.vd();
          AppMethodBeat.o(135754);
          paramInt = i;
          break;
        }
      }
      paramInt = -1;
      AppMethodBeat.o(135754);
      break;
    }
  }
}

/* Location:           C:\Users\Lin\Downloads\dex-tools-2.1-SNAPSHOT\dex-tools-2.1-SNAPSHOT\classes4-dex2jar.jar
 * Qualified Name:     com.tencent.mm.protocal.protobuf.rh
 * JD-Core Version:    0.6.2
 */