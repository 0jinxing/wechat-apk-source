package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import e.a.a.b;
import java.util.LinkedList;

public final class beb extends bsr
{
  public String jBB;
  public String luQ;
  public SKBuiltinBuffer_t wHH;
  public SKBuiltinBuffer_t wHI;

  public final int op(int paramInt, Object[] paramArrayOfObject)
  {
    int i = 0;
    AppMethodBeat.i(58917);
    if (paramInt == 0)
    {
      paramArrayOfObject = (e.a.a.c.a)paramArrayOfObject[0];
      if (this.wHH == null)
      {
        paramArrayOfObject = new b("Not all required fields were included: CurrentSynckey");
        AppMethodBeat.o(58917);
        throw paramArrayOfObject;
      }
      if (this.wHI == null)
      {
        paramArrayOfObject = new b("Not all required fields were included: MaxSynckey");
        AppMethodBeat.o(58917);
        throw paramArrayOfObject;
      }
      if (this.BaseRequest != null)
      {
        paramArrayOfObject.iy(1, this.BaseRequest.computeSize());
        this.BaseRequest.writeFields(paramArrayOfObject);
      }
      if (this.jBB != null)
        paramArrayOfObject.e(2, this.jBB);
      if (this.wHH != null)
      {
        paramArrayOfObject.iy(3, this.wHH.computeSize());
        this.wHH.writeFields(paramArrayOfObject);
      }
      if (this.wHI != null)
      {
        paramArrayOfObject.iy(4, this.wHI.computeSize());
        this.wHI.writeFields(paramArrayOfObject);
      }
      if (this.luQ != null)
        paramArrayOfObject.e(5, this.luQ);
      AppMethodBeat.o(58917);
      paramInt = i;
      return paramInt;
    }
    if (paramInt == 1)
      if (this.BaseRequest == null)
        break label864;
    label864: for (i = e.a.a.a.ix(1, this.BaseRequest.computeSize()) + 0; ; i = 0)
    {
      paramInt = i;
      if (this.jBB != null)
        paramInt = i + e.a.a.b.b.a.f(2, this.jBB);
      i = paramInt;
      if (this.wHH != null)
        i = paramInt + e.a.a.a.ix(3, this.wHH.computeSize());
      paramInt = i;
      if (this.wHI != null)
        paramInt = i + e.a.a.a.ix(4, this.wHI.computeSize());
      i = paramInt;
      if (this.luQ != null)
        i = paramInt + e.a.a.b.b.a.f(5, this.luQ);
      AppMethodBeat.o(58917);
      paramInt = i;
      break;
      if (paramInt == 2)
      {
        paramArrayOfObject = new e.a.a.a.a((byte[])paramArrayOfObject[0], unknownTagHandler);
        for (paramInt = bsr.getNextFieldNumber(paramArrayOfObject); paramInt > 0; paramInt = bsr.getNextFieldNumber(paramArrayOfObject))
          if (!super.populateBuilderWithField(paramArrayOfObject, this, paramInt))
            paramArrayOfObject.ems();
        if (this.wHH == null)
        {
          paramArrayOfObject = new b("Not all required fields were included: CurrentSynckey");
          AppMethodBeat.o(58917);
          throw paramArrayOfObject;
        }
        if (this.wHI == null)
        {
          paramArrayOfObject = new b("Not all required fields were included: MaxSynckey");
          AppMethodBeat.o(58917);
          throw paramArrayOfObject;
        }
        AppMethodBeat.o(58917);
        paramInt = i;
        break;
      }
      if (paramInt == 3)
      {
        Object localObject1 = (e.a.a.a.a)paramArrayOfObject[0];
        beb localbeb = (beb)paramArrayOfObject[1];
        paramInt = ((Integer)paramArrayOfObject[2]).intValue();
        int j;
        Object localObject2;
        boolean bool;
        switch (paramInt)
        {
        default:
          paramInt = -1;
          AppMethodBeat.o(58917);
          break;
        case 1:
          paramArrayOfObject = ((e.a.a.a.a)localObject1).Vh(paramInt);
          j = paramArrayOfObject.size();
          for (paramInt = 0; paramInt < j; paramInt++)
          {
            localObject2 = (byte[])paramArrayOfObject.get(paramInt);
            localObject1 = new hl();
            localObject2 = new e.a.a.a.a((byte[])localObject2, unknownTagHandler);
            for (bool = true; bool; bool = ((hl)localObject1).populateBuilderWithField((e.a.a.a.a)localObject2, (com.tencent.mm.bt.a)localObject1, bsr.getNextFieldNumber((e.a.a.a.a)localObject2)));
            localbeb.BaseRequest = ((hl)localObject1);
          }
          AppMethodBeat.o(58917);
          paramInt = i;
          break;
        case 2:
          localbeb.jBB = ((e.a.a.a.a)localObject1).BTU.readString();
          AppMethodBeat.o(58917);
          paramInt = i;
          break;
        case 3:
          localObject1 = ((e.a.a.a.a)localObject1).Vh(paramInt);
          j = ((LinkedList)localObject1).size();
          for (paramInt = 0; paramInt < j; paramInt++)
          {
            localObject2 = (byte[])((LinkedList)localObject1).get(paramInt);
            paramArrayOfObject = new SKBuiltinBuffer_t();
            localObject2 = new e.a.a.a.a((byte[])localObject2, unknownTagHandler);
            for (bool = true; bool; bool = paramArrayOfObject.populateBuilderWithField((e.a.a.a.a)localObject2, paramArrayOfObject, bsr.getNextFieldNumber((e.a.a.a.a)localObject2)));
            localbeb.wHH = paramArrayOfObject;
          }
          AppMethodBeat.o(58917);
          paramInt = i;
          break;
        case 4:
          localObject1 = ((e.a.a.a.a)localObject1).Vh(paramInt);
          j = ((LinkedList)localObject1).size();
          for (paramInt = 0; paramInt < j; paramInt++)
          {
            localObject2 = (byte[])((LinkedList)localObject1).get(paramInt);
            paramArrayOfObject = new SKBuiltinBuffer_t();
            localObject2 = new e.a.a.a.a((byte[])localObject2, unknownTagHandler);
            for (bool = true; bool; bool = paramArrayOfObject.populateBuilderWithField((e.a.a.a.a)localObject2, paramArrayOfObject, bsr.getNextFieldNumber((e.a.a.a.a)localObject2)));
            localbeb.wHI = paramArrayOfObject;
          }
          AppMethodBeat.o(58917);
          paramInt = i;
          break;
        case 5:
          localbeb.luQ = ((e.a.a.a.a)localObject1).BTU.readString();
          AppMethodBeat.o(58917);
          paramInt = i;
          break;
        }
      }
      paramInt = -1;
      AppMethodBeat.o(58917);
      break;
    }
  }
}

/* Location:           C:\Users\Lin\Downloads\dex-tools-2.1-SNAPSHOT\dex-tools-2.1-SNAPSHOT\classes4-dex2jar.jar
 * Qualified Name:     com.tencent.mm.protocal.protobuf.beb
 * JD-Core Version:    0.6.2
 */