...
public class ARParticleFilter extends VideoFilterBase
{
  ...
  private boolean changeTexture(String paramString)
  {
    ...
    while (true)
    {
      ...
      bool = false;
      continue;
      label257: AppMethodBeat.o(81666);
      bool = false;
    }
  }
  ...
  private void initParticle(List<String> paramList)
  {
    ...
    if (paramList != null)
    {
      ...
      for (i = 0; (i < this.mSpriteList.size()) && (i < this.mParticleDirList.size()); i++)
      {
        ...
        if (!TextUtils.isEmpty(paramFrameData.audioPath))
        {
          ...
          if (!this.mPlayerMapping.containsKey(str))
            if (!str.startsWith("assets://"))
              break label136;
          label136: for (paramFrameData = PlayerUtil.createPlayerFromAssets(VideoGlobalContext.getContext(), str.replace("assets://", ""), false); ; paramFrameData = PlayerUtil.createPlayerFromUri(VideoGlobalContext.getContext(), str, false))
          {
  ...
  public void addTouchPoint(PointF paramPointF)
  {
    ...
    Object localObject1 = new Point3D();
    ((Point3D)localObject1).x = (this.mViewDistance * ARMatrixUtil.cameraX);
    ((Point3D)localObject1).y = (this.mViewDistance * ARMatrixUtil.cameraY);
    ((Point3D)localObject1).z = (this.mViewDistance * ARMatrixUtil.cameraZ);
    Object localObject2 = new float[3];
    localObject2[0] = (ARMatrixUtil.cameraUpX * f1 + ARMatrixUtil.cameraRightX);
    localObject2[1] = (ARMatrixUtil.cameraUpY * f1 + ARMatrixUtil.cameraRightY);
    localObject2[2] = (ARMatrixUtil.cameraUpZ * f1 + ARMatrixUtil.cameraRightZ);
    vectorNormalization((float[])localObject2);
    f2 = (float)Math.sqrt(pow2(f2 * f3) + pow2(f4 * f5)) / 2.0F;
    Point3D localPoint3D1 = new Point3D();
    localPoint3D1.x = (((Point3D)localObject1).x - localObject2[0] * f2);
    localPoint3D1.y = (((Point3D)localObject1).y - localObject2[1] * f2);
    localPoint3D1.z = (((Point3D)localObject1).z - localObject2[2] * f2);
    ...
    ((Point3D)localObject1).x += arrayOfFloat[0] * f2;
    ((Point3D)localObject1).y += arrayOfFloat[1] * f2;
    ((Point3D)localObject1).z += arrayOfFloat[2] * f2;
    Point3D localPoint3D2 = new Point3D();
    localPoint3D2.x = (((Point3D)localObject1).x - arrayOfFloat[0] * f2);
    localPoint3D2.y = (((Point3D)localObject1).y - arrayOfFloat[1] * f2);
    localPoint3D2.z = (((Point3D)localObject1).z - f2 * arrayOfFloat[2]);
    f2 = paramPointF.x / this.width;
    f1 = f2;
    if (ARMatrixUtil.isFrontCamera)
      f1 = 1.0F - f2;
    localObject1 = new float[3];
    localObject1[0] = ((localPoint3D2.x - localPoint3D1.x) * f1);
    localObject1[1] = ((localPoint3D2.y - localPoint3D1.y) * f1);
    localObject1[2] = (f1 * (localPoint3D2.z - localPoint3D1.z));
    f1 = 1.0F - paramPointF.y / this.height;
    paramPointF = new float[3];
    paramPointF[0] = ((((Point3D)localObject2).x - localPoint3D1.x) * f1);
    paramPointF[1] = ((((Point3D)localObject2).y - localPoint3D1.y) * f1);
    paramPointF[2] = (f1 * (((Point3D)localObject2).z - localPoint3D1.z));
    localObject2 = new Point3D();
    ((Point3D)localObject2).x = (localPoint3D1.x + localObject1[0] + paramPointF[0]);
    ((Point3D)localObject2).y = (localPoint3D1.y + localObject1[1] + paramPointF[1]);
    f1 = localPoint3D1.z;
    ((Point3D)localObject2).z = (localObject1[2] + f1 + paramPointF[2]);
    this.mParticleSystem.emitImmediately(((Point3D)localObject2).x, ((Point3D)localObject2).y, ((Point3D)localObject2).z);
  ...
  public void clear()
  {
    ...
    for (Bitmap localBitmap : this.mTextureBitmaps)
