...
public class TransitionBlendFilter extends BaseFilter
  implements ITransitionFilter
{
  ...
  private void updateTextureParam(long paramLong)
  {
    ...
    {
      return;
      localBitmap = getNextFrame((int)((()getDuration(this.transitionStartTime, this.transitionStartTime + this.transitionDuration, paramLong) - this.transitionStartTime) / Math.max(this.item.frameDuration, 1.0D)) % this.item.frames);
    ...
    locall.bMS = localBitmap;
    if ((locall.bMS != null) && (!locall.bMS.isRecycled()));
