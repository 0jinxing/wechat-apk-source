...
public class ActFilters
{
  ...
  static
  {
    ...
    actItemComparator = new ActFilters.1();
  ...
  private List<CanvasItem> getCanvasItems(List<CanvasItem> paramList, long paramLong)
  {
    ...
    while (true)
    {
      ...
      {
        paramList = (CanvasItem)localIterator.next();
        if ((paramList.startTime <= paramLong) && (paramLong <= paramList.endTime))
  ...
  public h updateAndRender(h paramh, long paramLong, List<List<PointF>> paramList, List<float[]> paramList1, int paramInt)
  {
    ...
    while (true)
    {
      ...
      render(i);
      paramList1 = AlgoUtils.calPositions(new Rect(0, 0, paramh.width, paramh.height), this.mCanvasFrame.width, this.mCanvasFrame.height, paramh.width, paramh.height, this.faceExpression.canvasResizeMode);
      paramList = AlgoUtils.calTexCords(new Rect(0, 0, paramh.width, paramh.height), this.mCanvasFrame.width, this.mCanvasFrame.height, this.faceExpression.canvasResizeMode);
      if (needResize(paramList1, paramList))
      {
        BenchUtil.benchStart(TAG + "[resize]");
        this.mCopyFilter.setPositions(paramList1);
        this.mCopyFilter.setTexCords(paramList);
