...
public class PTBeautyFaceList
{
  ...
  public h process(h paramh, PTFaceAttr paramPTFaceAttr)
  {
    ...
    if (this.mFaceFeatureFilter != null)
    {
      ...
      {
        localObject = (List)localList1.get(j);
        paramPTFaceAttr = (float[])localList2.get(j);
        paramPTFaceAttr = new PTDetectInfo.Builder().facePoints((List)localObject).faceAngles(paramPTFaceAttr).faceActionCounter(localMap).handPoints(localList3).handActionCounter(GestureDetector.getInstance().getHandActionCounter()).triggeredExpression(localSet).phoneAngle(i).timestamp(l).build();
    ...
    if (this.mEyeLightenFilter != null)
    {
      ...
      if (this.mEyeLightenFilter.needRender())
      {
        ...
        for (j = 0; j < localList1.size(); j++)
        {
          paramh = (List)localList1.get(j);
          localObject = (float[])localList2.get(j);
          paramh = new PTDetectInfo.Builder().facePoints(paramh).faceAngles((float[])localObject).faceActionCounter(localMap).handPoints(localList3).handActionCounter(GestureDetector.getInstance().getHandActionCounter()).triggeredExpression(localSet).phoneAngle(i).timestamp(l).build();
