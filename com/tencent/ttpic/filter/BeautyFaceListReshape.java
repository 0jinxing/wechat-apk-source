    ...
    for (Map localMap = paramFaceDetector.getFaceActionCounter(); ; localMap = null)
    {
      ...
      if (this.mFaceFeatureFilter != null)
      {
        ...
        if (this.mFaceFeatureFilter.needRender())
        {
          ...
          for (i = 0; i < paramList.size(); i++)
          {
            ...
            paramFaceDetector = (float[])paramList1.get(i);
            paramFaceDetector = new PTDetectInfo.Builder().facePoints(localList).faceAngles(paramFaceDetector).faceActionCounter(localMap).handPoints(paramList2).handActionCounter(GestureDetector.getInstance().getHandActionCounter()).triggeredExpression(paramSet).phoneAngle(paramInt).timestamp(paramLong).build();
      ...
      if (this.mEyeLightenFilter != null)
      {
        ...
        if (this.mEyeLightenFilter.needRender())
        {
          ...
          {
            localList = (List)paramList.get(i);
            paramh = (float[])paramList1.get(i);
            paramh = new PTDetectInfo.Builder().facePoints(localList).faceAngles(paramh).faceActionCounter(localMap).handPoints(paramList2).handActionCounter(GestureDetector.getInstance().getHandActionCounter()).triggeredExpression(paramSet).phoneAngle(paramInt).timestamp(paramLong).build();
